  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-graphite-200 tracking-tight">Session Explorer</h2>
          <p className="text-sm text-graphite-500">{sessions.length} flows grouped by five-tuple</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <input value={filter} onChange={e => setFilter(e.target.value)}
              ...
              className="bg-graphite-800 border border-graphite-700/40 rounded-lg pl-8 pr-3 py-1.5 text-sm text-graphite-300 placeholder:text-graphite-600 w-56 outline-none focus:border-accent/40 transition-colors" />
          </div>
        </div>
      </div>
        ...
        {filtered.map((s, i) => (
          <motion.div key={s.id}
            ...
            <button onClick={() => setExpanded(expanded === s.id ? null : s.id)}
              ...
              {/* Flow */}
              <span className="font-mono text-xs text-graphite-300 min-w-[260px] shrink-0">
              ...
              {/* SNI */}
              {s.sni && (
                <span className="font-mono text-xs text-graphite-400 truncate max-w-[160px]">{s.sni}</span>
              )}
              ...
              <span className="ml-auto flex items-center gap-3 shrink-0">
                ...
                <span className="font-mono text-xs text-graphite-600 tabular-nums w-16 text-right">{s.packetCount} pkts</span>
                <span className="font-mono text-xs text-graphite-600 tabular-nums w-16 text-right">{s.totalBytes} B</span>
            ...
            <AnimatePresence>
              {expanded === s.id && (
                <motion.div
                  ...
                  <div className="border-t border-graphite-800/60 px-4 py-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 text-xs bg-graphite-950/30">
                    {[
                      ...
                      <div key={label}>
                        ...
                        <p className="font-medium font-mono" style={{ color: color || '#b0b8c4' }}>{val}</p>
                      </div>
                    ))}
                    {s.sni && (
                      <div className="col-span-2">
                        <p className="text-[10px] uppercase tracking-wider text-graphite-600 mb-0.5">SNI Domain</p>
                        <p className="text-accent font-mono">{s.sni}</p>
                      </div>
                    )}
