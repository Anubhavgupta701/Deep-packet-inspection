...
│   ├── pcap_reader.h          # PCAP file reading //recordings of packets in computer by wireshark 
...
└── README.md                  # This file!
```
//my understanding :{Imagine

Tum YouTube kholte ho.

Browser ko internet pe message bhejna hai:

"Mujhe www.youtube.com chahiye"

Ye message directly hawa me nahi jata.

Usko packing kiya jata hai.

STEP 1 — HTTPS Message Bana

Browser ek HTTPS/TLS message banata.
...
SNI
STEP 2 — Is Message Ko Carry Kaun Karega?

Message khud travel nahi kar sakta.

Usko ek transport chahiye.
...
Oh YouTube hai
Block karo
Sabse Important Sentence
SNI TCP ke andar hota hai.

More exactly:

TCP payload ke andar TLS hota
TLS ke andar SNI hota}
---
...
```
raw.data bytes:
[0-13]   Ethernet Header //ghr k andr kis device ko dena h
[14-33]  IP Header  //city+house addrss
[34-53]  TCP Header // ghr k andr kis person ko
[54+]    Payload //actual data
...
*Network Byte Order:* Network protocols use big-endian (most significant byte first). Your computer might use little-endian. We use `ntohs()` and `ntohl()` to convert numbers from high bits to low bits for easily readable by cpus:
```cpp
...
//basic commands running the code
# DPI Engine Commands Cheat Sheet (Windows)

## Go to Project Folder

```powershell
cd C:\Users\hp\Downloads\Packet_analyzer-main\Packet_analyzer-main
```
...
```powershell
.\dpi_engine.exe test_dpi.pcap output.pcap --block-app TikTok
```
...
```powershell
.\dpi_engine.exe test_dpi.pcap output.pcap --block-app YouTube --block-app TikTok
```
...
```powershell
.\dpi_engine.exe test_dpi.pcap output.pcap --block-domain facebook
```

```powershell
.\dpi_engine.exe test_dpi.pcap output.pcap --block-domain google
```
...
```powershell
.\dpi_engine.exe test_dpi.pcap output.pcap --block-ip 8.8.8.8
```

```powershell
.\dpi_engine.exe test_dpi.pcap output.pcap --block-ip 192.168.1.50
```
...
```powershell
.\dpi_engine.exe test_dpi.pcap output.pcap --block-ip 8.8.8.8 --block-ip 1.1.1.1
```
...
```powershell
.\dpi_engine.exe test_dpi.pcap output.pcap --lbs 4 --fps 4
```
...
```powershell
.\dpi_engine.exe test_dpi.pcap output.pcap --block-app YouTube --block-domain facebook --block-ip 8.8.8.8 --lbs 2 --fps 4
```
